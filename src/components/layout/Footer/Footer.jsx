...
const FooterLinkSection = ({ title, links }) => (
  <div>
    ...
    <ul className="mt-6 space-y-4 text-base">
      {links.map(link => (
        <li key={link}>
          <a href="#" className="text-gray-700 font-semibold transition hover:opacity-75">{link}</a>
        </li>
      ))}
    </ul>
  </div>
...
function Footer() {
  const sections = [
    { title: 'Community', links: ['1on1 Coaching', 'Company Review', 'Accounts Review', 'HR Consulting', 'SEO Optimisation'] },
      ...
      <div className="flex flex-col items-center justify-center   px-8 pt-8 pb-4    ">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3 lg:space-x-96">
          <div className='w-full'>
            <div className=" text-4xl font-bold">Hackfolic<span className="text-blue-500">.</span>
                <span className="text-green-500">.</span>
                <span className="text-gray-400">.</span></div>
            <p className="mt-4 max-w-xs text-gray-700 text-3xl font-bold  ">
            We love <span className='text-blue-500'>software</span> and the <span className='text-green-500'>people</span> who build them.
            </p>
          </div>
        ...
        <div className="text-xs text-gray-700 mt-4 text-center">© 2022. Company Name. All rights reserved.</div>
      </div>
    </footer>
  );
}
